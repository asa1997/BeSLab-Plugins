...
function __beslab_install_Scorecard() {
...
function __beslab_uninstall_Scorecard() {

	__besman_echo_no_colour "Stopping and removing $BESLAB_OIAB_BUYER_APP"
	cd "$BESLAB_OIAB_BUYER_APP_DIR" || return 1
	isInstalled=$(docker image ls | grep -i scorecard | awk '{print $1}' | cut -d'/' -f3)

	if [[ "${isInstalled}"== "scorecard" ]];then
           docker rmi -f gcr.io/openssf/scorecard:stable
	fi
	cd "$HOME" || return 1
}

function __beslab_plugininfo_Scorecard()
{
	cat <<EOF
### Plugin Information
...
This plugin is to install scorecard utility to assess the github repository score.
...
bli install plugin Scorecard 0.0.1
